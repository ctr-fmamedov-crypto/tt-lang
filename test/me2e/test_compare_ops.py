...
"""
ME2E tests for elementwise compare operations (ttl.gt, ttl.lt, ttl.eq, ttl.ne).

Lowering uses ttkernel.{gt,lt,eq,ne}_binary_tile (LLK binary SFPU compare;
same kernel family as ttnn.{gt,lt,eq,ne}). Golden references use
torch.{gt,lt,eq,ne}; bool tensors are converted to float 0/1 for device I/O.
...
BF16 compare cases are skipped (exact mask vs golden). Float32 1x1 ``lt`` / ``eq`` /
``ne`` are skipped (exact mask mismatch vs torch golden on that grid; ``gt`` still runs).
...
_COMPARE_OPS = tuple(op for op in COMPUTE_OPS if op.name in ("gt", "lt", "eq", "ne"))
assert len(_COMPARE_OPS) == 4, (
    "COMPUTE_OPS must include exactly gt/lt/eq/ne; check ELEMENTWISE_OPS / OP_TORCH_MAP "
...
def _compare_test_params():
    ...
    for op, config, dtype in itertools.product(
        ...
        skip_f32_1x1_compare = pytest.mark.skipif(
            ...
            reason=(
                "Compare ME2E: skip float32 1x1 lt/eq/ne (exact mask vs torch golden)."
            ),
        )
        params.append(
